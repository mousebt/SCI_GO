# 脑卒中相关性肺炎（SAP）常规多模态数据 1:4 匹配嵌套病例对照研究方案
## (基于两篇顶刊方法学融合：Zhang et al., 2026 + Yu et al., 2024)

**项目代号**：`SAP-Routine-NCC-Study`  
**版本**：v1.0 (2026-08-18)  
**设计类型**：单中心回顾性队列 1:4 嵌套病例对照研究（1:4 Matched Nested Case-Control Study）  
**数据基底**：200 余例脑卒中住院患者历史常规病历、化验单与护理记录（SAP 发生率 < 3%）  
**方法学对标**：
1. *Acta Microbiol Immunol Hung (2026, Zhang et al.)*: 增量价值分析（$\Delta\text{AUC}$、NRI、IDI、Brier 评分）与复合评分构建；
2. *Front. Endocrinol. (2024, Yu et al., PMID: 39588332)*: 常规检验（WBC/CRP/生化）、吞咽量表（KWDT/GCS）与时序生存分析（Kaplan-Meier）。
...
2. **现有工具瓶颈**：临床常用的 $A_2DS_2$、AIS-APS 等评分虽被广泛使用，但在低发生率、轻中度卒中病区存在“区分度不足（AUC 往往低于 0.80）”与“校准度漂移”的局限。
3. **创新切入点**：无需开展高成本的新化验，直接挖掘患者入院 24h 内已有的**常规血常规（NLR/SII）、常规生化（白蛋白/CRP/eGFR）及常规护理文书（首次吞咽筛查时效、GCS、洼田饮水试验 KWDT）**，通过 1:4 嵌套匹配与增量价值分析，证实其对 $A_2DS_2$ 具有显著的风险重分类与预测增量价值。

---

## 二、 研究对象与 1:4 匹配流程（Flowchart）
...
```
                    ...
                    ┌────────────────┴────────────────┐
                    ▼                                 ▼
            【病例组：全部 6 例 SAP】         【对照池：~194 例未发生 SAP】
                    │                                 │
                    └────────────────┬────────────────┘
                                     ▼
      【以 1:4 比例进行严格风险集匹配（Risk-Set Matched Sampling）】
         • 匹配变量 1：年龄（±3 岁）
         • 匹配变量 2：性别（100% 精确同性）
         ...
         • 匹配变量 4：卒中亚型（脑梗死 vs 脑出血 100% 相同）
                    ...
                    ┌────────────────┴────────────────┐
                    ▼                                 ▼
              6 例 SAP 患者                     24 例精准匹配的 Control
                    │                                 │
                    └────────────────┬────────────────┘
                                     ▼
                【最终核心分析数据集：总计 30 例深度比对】
```
