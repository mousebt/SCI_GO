...
The source analysis retained 14,780 eligible reports, 42,808 adverse-event records and 951 preferred terms. The SQL output reportedly identified 24 positive preferred-term signals, while the Chinese manuscript displayed 20 rows according to its table-presentation convention. Extravasation-related terms showed the strongest disproportionality. Shock, acute pulmonary oedema, renal tubular necrosis, dyspnoea and acute kidney injury were also clinically relevant. Among 12,588 reports with evaluable timing, 81.56% occurred within 24 hours. Local data are pending. `[VERIFY all FAERS and local data after rerunning the submission SQL, including EBGM/EBGM05 outputs.]`
...
Eligible reports were those listing ioversol, iohexol, iopamidol, or iodixanol as the primary suspect drug. The archived SQL target list contains the generic names Ioversol, Iohexol, Iopamidol and Iodixanol. Brand names, spelling variants, salts and exclusion terms appear to have been handled upstream through the cleaned drug-name table and should be documented separately. `[AUTHOR INPUT REQUIRED: provide the drug_clean construction rules or a separate drug dictionary export.]`
...
The source manuscript lists the following screening rules: a minimum of three reports, lower 95% confidence limit for ROR greater than 1, PRR greater than 2 with chi-square at least 4, IC025 greater than 0, and EBGM05 greater than 2. Formulae are retained in the manuscript methods, and complete algorithm outputs will be supplied in tables. The archived SQL calculates EBGM and an MGPS lower-bound variable named EB05; this should be exported as EBGM05 if it is the intended lower confidence bound. `[AUTHOR INPUT REQUIRED: add EB05/EBGM05 and MGPS signal flags to the exported SOC/PT tables and confirm whether final signal positivity requires IC025 and EBGM05, because the current final SQL export filters ROR and PRR only.]`

Signal magnitude was interpreted as the strength of disproportionate reporting, not as event probability, clinical severity, or causal effect. The current working draft treats the four contrast agents as a pooled ICM exposure group. The archived SQL currently lists all four drugs together; true agent-specific results would require separate runs for each agent or grouping by drug name. If feasible, agent-specific outputs will be reported as sensitivity or supplementary analyses; otherwise, the manuscript will justify pooling and avoid comparative claims between agents.

### 2.7 Time-to-onset analysis

TTO was calculated using pre-cleaned date fields as the interval between the recorded AE date (`demo_clean.event_dt`) and treatment start date (`ther.start_dt`). The archived SQL retained records in which both dates used an eight-digit format and `tto_days` was between 0 and 730. TTO was summarised using descriptive statistics and modelled with a Weibull distribution. A shape parameter below 1 was interpreted as an early-failure pattern, indicating that reports with evaluable timing were concentrated early after administration. `[AUTHOR INPUT REQUIRED: provide the same-day/24-hour export, partial-date rule, median and interquartile range, Weibull confidence intervals, and fitting procedure.]`
